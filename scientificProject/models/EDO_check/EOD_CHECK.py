...
DAY_EOD_path=r"P:\Operations\Polymer - Middle Office\EOD Check\\"
...
for DAY_EOD_file in DAY_EOD_path_dir:
    if DAY_EOD_file.startswith("EOD TRADE BLOTTER CHECKER " + today_str) and not DAY_EOD_file.endswith("- Overnight.xlsx"):  #exculde the overnight file
        DAY_EOD_path = os.path.join(DAY_EOD_path, DAY_EOD_file)
        break

# hard code
# DAY_EOD_path = "P:\Operations\Polymer - Middle Office\EOD Check\Christy\INPUT\EOD TRADE BLOTTER CHECKER 2023-08-21.xlsx"
# yesterday_str="2023-08-21"

print(DAY_EOD_path)

# extract both pm_mappaing data, trader list & EOD booking data
DAY_EOD_File=pd.read_excel(DAY_EOD_path, sheet_name="Sheet2")
...
def HIERARCHY_T_F(row):
    ...
    elif TORA == "TORA-DROPCOPY" and HIERARCHY == Business_Unit or (HIERARCHY in SBU_LIST and HIERARCHY == SUB_Business_Unit):
        return "TRUE"
    else:
...
def FINANCIAL_REQUIRED(SWAP, Custodian):
    if 'ISDA' in Custodian: 
        return "Required"
    else:
        return "Not Required"
...
DAY_EOD_File['Financing Description Requirement'] = DAY_EOD_File.apply(
    lambda row: FINANCIAL_REQUIRED(row['SWAP ORDER (NEW)'], row['Custodian Name']),
    axis=1
)
...
with pd.ExcelWriter("P:\Operations\Polymer - Middle Office\EOD Check\Christy\EOD_CHECK"+today_str+"_DAY.xlsx") as writer:
    ...
    if LENGTH_HIERARCHY_F != 0:
        ...
        False_Check_Final.to_excel(writer, sheet_name="EMAIL SUMMARY", index=False, startrow=i+LENGTH_EMAIL_MAIN_F)
    if LENGTH_ACCT_F != 0:
        i = i+2
        AF=[["Acct/Fund Check:"]]
        ...
        False_Check_ACCT_Final.to_excel(writer, sheet_name="EMAIL SUMMARY", index=False, startrow=i+LENGTH_EMAIL_MAIN_F+LENGTH_HIERARCHY_F)
    if LENGTH_BORROW_F != 0:
        ...
        False_Check_BORROW_Final.to_excel(writer, sheet_name="EMAIL SUMMARY", index=False, startrow=i+LENGTH_EMAIL_MAIN_F+LENGTH_HIERARCHY_F+LENGTH_ACCT_F)
    if LENGTH_FINA_F != 0:
        i = i+2
        FA=[["Preallocation Check:"]]
        FA_df = pd.DataFrame(FA, columns =["Column1"], index=[1])
        list_email.append(FA_df)
        list_email.append(False_Check_FINA_FINAL)
        False_Check_FINA_FINAL.to_excel(writer, sheet_name="EMAIL SUMMARY", index=False, startrow=i+LENGTH_EMAIL_MAIN_F+LENGTH_HIERARCHY_F+LENGTH_ACCT_F+LENGTH_BORROW_F)
    if LENGTH_PRE_F != 0:
        i = i+2
        ...
        False_Check_PRE_FINAL.to_excel(writer, sheet_name="EMAIL SUMMARY", index=False, startrow=i+LENGTH_EMAIL_MAIN_F+LENGTH_HIERARCHY_F+LENGTH_ACCT_F+LENGTH_BORROW_F+LENGTH_FINA_F)
    if LENGTH_MAIN_F != 0:
        i = i+2
        ...
        False_Check_MAIN_Final.to_excel(writer, sheet_name="EMAIL SUMMARY", index=False, startrow=i+LENGTH_EMAIL_MAIN_F+LENGTH_HIERARCHY_F+LENGTH_ACCT_F+LENGTH_BORROW_F+LENGTH_FINA_F+LENGTH_PRE_F)        
    if LENGTH_SWAP_F != 0:
        i = i+2
        SWAP_C=[["Swap Check:"]]
        ...
        False_Check_SWAP_Final.to_excel(writer, sheet_name="EMAIL SUMMARY", index=False, startrow=i+LENGTH_EMAIL_MAIN_F+LENGTH_HIERARCHY_F+LENGTH_ACCT_F+LENGTH_BORROW_F+LENGTH_FINA_F+LENGTH_PRE_F+LENGTH_MAIN_F)        
 
# JUST try some fancy stuff        
import openpyxl
export_path ="P:\Operations\Polymer - Middle Office\EOD Check\Christy\EOD_CHECK"+today_str+"_DAY.xlsx"
...
text_file = open("P:\Operations\Polymer - Middle Office\EOD Check\Christy\TEST_2.htm",'w')
...
sub = "EOD_CHECK_" + today_str
mail.Subject = sub
# mail.Body = 'Message body testing'

HtmlFile = open(r'P:\Operations\Polymer - Middle Office\EOD Check\Christy\TEST_2.htm', 'r')
