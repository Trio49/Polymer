...
namemapping = {"ECMIPO":"ECMIP","MAIN":"MAIN_","ANDNG":"ANDNG","DSING":"DSING","EDMLO":"EDMLO","SLEUN":"SLEUN"}
...
#gs margin reports
GS_path=r"\\paghk.local\shares\Enfusion\polymer_reports\GS\\"+yesterday_str
...
PM_Margin_Output.columns = ["MR(INR)","MR(USD)","AVG MR%","MARGIN EXCESS (INR)","MARGIN EXCESS (USD)",
...
output_final = output_step2[["$ Gross Exposure","MR(USD)","AVG MR%",
                             "SecMTM","CashBalance",
                             "MR(INR)","MARGIN EXCESS (INR)","MARGIN EXCESS (USD)",
...
output_final.loc["ROLL-UP","AVG MR%"] = -output_final.loc["ROLL-UP","MR(USD)"]/output_final.loc["ROLL-UP","$ Gross Exposure"]

output_final.loc["ROLL-UP","Upper Limit - 80%"] = output_final["Upper Limit - 80%"][:-1].sum()
output_final.loc["ROLL-UP","Lower Limit - 50%"] = output_final["Lower Limit - 50%"][:-1].sum()
...
mail.To = 'tyin@pag.com;alanw@pag.com'

#mail.CC = PM_enail_mapping_CC_list
sub = "India FPI F&O Margin Summary - POLYMER - For Close of BDay " + yesterday_str
