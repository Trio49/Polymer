...
#ASHAH only
# MTM_File = MTM_File[MTM_File["Custodian Account Display Name"].str.contains("ASHAH")]
#exclude Settled Cash - USD under Description
MTM_File = MTM_File[~MTM_File["Description"].str.contains("Settled Cash - USD")]
...
for pmkey in PM_path.keys():
    ...
    #read a list of pmkeys where the $ Gross Exposure is not 0


    if pmkey in list(raw_pos_group_gmv_df[raw_pos_group_gmv_df["$ Gross Exposure"]!=0].index):
...
#for alert=='WITHDRAW", the amount is the difference between EOD_BAL_USD and lowerlimit50
    if alert == "WITHDRAW":
...
mail.To = 'dchau@polymercapital.com;johnathanl@polymercapital.com;zzhang@polymercapital.com'
