...
if ytd == 'yes':
    ...
    poly_fin = pd.read_csv(r'P:\All\FinanceTradingMO\Rebate_Borrow breakdown\POLY_FINANCING_II_202308.csv',
...
#save to csv
poly_fin_grouped.to_csv(r'P:\Operations\Polymer - Middle Office\Borrow analysis\Bloomberg data\poly_fin_grouped.csv')
...
if check == 'yes':
    ...
    poly_fin_top20variance = poly_fin_stddev.sort_values(by='Std Dev', ascending=False).head(20)
...
def generate_plots():
    ...
    top5_volatile_names = pd.read_csv(
        r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/top20_MTD_ticker_rate_variance.csv")
    #remove rows with CLSA in PB
    # top5_volatile_names = top5_volatile_names[~top5_volatile_names['PB'].str.contains('CLSA')]
    top5_volatile_names = top5_volatile_names.head(20)
    # add holding scenartio to poly_fin_grouped too
    poly_fin_grouped= pd.read_csv(r'P:\Operations\Polymer - Middle Office\Borrow analysis\Bloomberg data\poly_fin_grouped.csv')
    ...
    poly_fin_grouped_top5 = poly_fin_grouped_top5[['BB Ticker','Holding Scenario', 'Date2', 'Weighted Broker Fee']]

    # for each holding scenario, scatter the weighted broker fee
    #read benchmark in the same folder and merge with poly_fin_grouped_top5
    benchmark=pd.read_csv(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/Benchmark.csv")
    #merge poly_fin_grouped_top5 with benchmark
    poly_fin_grouped_top5=poly_fin_grouped_top5.merge(benchmark,how='left',left_on=['BB Ticker','Date2'],right_on=['bbgYellowKey','date'])
    #remove Weighted Broker Fee=0
    poly_fin_grouped_top5=poly_fin_grouped_top5[poly_fin_grouped_top5['Weighted Broker Fee']!=0]
    #keep the columns needed
    poly_fin_grouped_top5=poly_fin_grouped_top5[['Holding Scenario', 'Date2', 'Weighted Broker Fee','Benchmark']]
    ...
    from matplotlib import pyplot as plt
    poly_fin_grouped_top5=pd.read_csv(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/plots_raw_data.csv")
    for i in poly_fin_grouped_top5['Holding Scenario'].unique():
        poly_fin_grouped_top5[poly_fin_grouped_top5['Holding Scenario'] == i].plot(x='Date2', y=['Weighted Broker Fee','Benchmark'],
                                                                                   title=i, kind='line')
        plt.show()
    # save the plots to folder with holding scenario name plus the month as the file name

    # poly_fin_groupd_top5=pd.read_csv(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/plots_raw_data.csv")
    for i in poly_fin_grouped_top5['Holding Scenario'].unique():
        poly_fin_grouped_top5[poly_fin_grouped_top5['Holding Scenario'] == i].plot(x='Date2', y=['Weighted Broker Fee','Benchmark'],
                                                                                   title=i, kind='line')
        plt.savefig(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots\\" + i + ".png")
        plt.close()
    return(poly_fin_grouped_top5)
poly_fin_grouped_top5=generate_plots()
print('done')