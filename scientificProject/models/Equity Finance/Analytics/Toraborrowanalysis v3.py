...
if combine == "yes":
    os.chdir(r"P:\Operations\Polymer - Middle Office\Borrow analysis\Tora data iutput")
    extension = 'csv'
    ...
    # export to csv
    # combined_csv.to_csv( "combined_csv.csv", index=
...
df = pd.read_csv(r"P:\Operations\Polymer - Middle Office\Borrow analysis\Tora data iutput/combined_csv.csv",
                 engine='python')
# df=pd.read_csv(r"T:\Daily trades\Daily Re\SmartAllo\Borrowinput\POLYMER_BORROW_1615_20220204.csv",engine='python')
Locates = df[['BloombergCode', 'State', 'RequestOty', 'ApprovedQty', 'Rate', 'Broker', 'RecDate', 'ExecTimestamp',
              'InternalAccount']]
...
#define the date range from date_min to date_max
dates_min="20230801"
dates_max="20230831"
print("start date is ", dates_min, "end date is " ,dates_max)
px_list3 = blp.bdh(Ticker_list, 'PX_LAST', "20230801", "20230831", Currency='USD')
...
Locates4 = Locates3.merge(px_list3, left_on=['BloombergCode2', 'RecDate'], right_on=['BloombergCode', 'RecDate'],
                          how='left')
# data massage
# df[df['A'].isin([3, 6])
Locates4 = Locates4[Locates4['Broker'].isin(['gs', 'baml', 'jpm', 'msdw', 'ubs', 'cicc'])]
Locates4['MV'] = Locates4['ApprovedQty'] * Locates4['HistPrice']
Locates4['BloombergCode'] = Locates4['BloombergCode_x']
Locatesdata = Locates4[
    ['BloombergCode', 'BloombergCode2', 'State', 'RequestOty', 'ApprovedQty', 'Rate', 'Broker', 'RecDate',
     'ExecTimestamp', 'InternalAccount', 'EXCH_CODE', 'Currency', 'HistPrice', 'MV']]
...
Locatesdata.to_csv(r"P:\Operations\Polymer - Middle Office\Borrow analysis\Toradata.csv", index=False,
                   line_terminator='\n')
Cantfill.to_csv(r"P:\Operations\Polymer - Middle Office\Borrow analysis\nofill.csv", index=False, line_terminator='\n')
Cantfillcount.to_csv(r"P:\Operations\Polymer - Middle Office\Borrow analysis\nofillcount.csv", index=False,
                     line_terminator='\n')
print('done')