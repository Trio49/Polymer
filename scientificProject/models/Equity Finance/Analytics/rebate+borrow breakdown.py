...
if check == 'yes':
    ...
    MTD_PB_Swap.to_csv(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\MTD_PB_Swap" + month + ".csv", index=False,
                       lineterminator='\n')
    MTD_PM_Swap.to_csv(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\MTD_PM_Swap" +"__"+ month + ".csv", index=False,
                       lineterminator='\n')
    MTD_PB_Cash.to_csv(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\MTD_PB_Cash" + "__"+month + ".csv", index=False,
                       lineterminator='\n')
    MTD_PM_Cash.to_csv(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdown\MTD_PM_Cash" +"__"+ month + ".csv", index=False,
                       lineterminator='\n')
    print('Saved')
    # to mark overwrite file
    # create an Excel file and ave the 4 sheets in it
    # month = input('which month please:  ')
    import pandas as pd
    from openpyxl import load_workbook
    with pd.ExcelWriter(r'P:\All\FinanceTradingMO\Rebate_Borrow breakdown/202307_Financing.xlsx') as writer:
...
def generate_MTDPB_chart():
    MTD_PB_Swap_Chart=MTD_PB_Swap
    #Remove column month
    MTD_PB_Swap_Chart=MTD_PB_Swap_Chart.drop(['Month'],axis=1)
    #set index to PB
    MTD_PB_Swap_Chart=MTD_PB_Swap_Chart.set_index('PB')
    MTD_PB_Swap_Chart=MTD_PB_Swap_Chart.astype(float)
    MTD_PB_Swap_Chart.plot(kind='bar',figsize=(20,10))
    plt.legend(title='MTD_PB_Swap')
    plt.title('MTD_PB_Swap')

    #save the plot as a picture
    plt.savefig(r'P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/MTD_PB_Swap')
    plt.show()
    plt.close()
    return MTD_PB_Swap_Chart,plt
MTD_PB_Swap_Chart,plt=generate_MTDPB_chart()

#plot for MTD_PM_Swap
def generate_MTDPMSwap_chart():
    MTD_PM_Swap_CHARt=MTD_PM_Swap
    #Remove column month
    MTD_PM_Swap_CHARt=MTD_PM_Swap_CHARt.drop(['Month'],axis=1)
    #set index to PB
    MTD_PM_Swap_CHARt=MTD_PM_Swap_CHARt.set_index('PM')
    MTD_PM_Swap_CHARt=MTD_PM_Swap_CHARt.astype(float)
    MTD_PM_Swap_CHARt.plot(kind='bar',figsize=(20,10))
    plt.legend(title='MTD_PM_Swap')
    plt.title('MTD_PM_Swap')
    #save the plot as a picture
    plt.savefig(r'P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/MTD_PM_Swap')
    plt.show()
    plt.close()
    return MTD_PM_Swap_CHARt,plt
MTD_PM_Swap_CHARt,plt=generate_MTDPMSwap_chart()

#plot for MTD_PB_Cash and MTD_PM_Cash
def generate_MTDPB_Cash_chart():
    MTD_PB_Cash_Chart=MTD_PB_Cash
    #Remove column month
    MTD_PB_Cash_Chart=MTD_PB_Cash_Chart.drop(['Month'],axis=1)
    #set index to PB
    MTD_PB_Cash_Chart=MTD_PB_Cash_Chart.set_index('PB')
    MTD_PB_Cash_Chart['MTD Short Fee Accrual']=MTD_PB_Cash_Chart['MTD Short Fee Accrual'].abs()
    MTD_PB_Cash_Chart=MTD_PB_Cash_Chart.astype(float)
    MTD_PB_Cash_Chart.plot(kind='bar',figsize=(20,10))
    plt.legend(title='MTD_PB_Cash')
    plt.title('MTD_PB_Cash')
    #save the plot as a picture
    plt.savefig(r'P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/MTD_PB_Cash')
    plt.show()
    plt.close()
    return MTD_PB_Cash_Chart,plt
MTD_PB_Cash_Chart,plt=generate_MTDPB_Cash_chart()
def generate_MTDPM_cash_chart():
    MTD_PM_Cash_Chart=MTD_PM_Cash
    #Remove column month
    MTD_PM_Cash_Chart=MTD_PM_Cash_Chart.drop(['Month'],axis=1)
    #set index to PB
    MTD_PM_Cash_Chart=MTD_PM_Cash_Chart.set_index('PM')
    MTD_PM_Cash_Chart['MTD Short Fee Accrual'] = MTD_PM_Cash_Chart['MTD Short Fee Accrual'].abs()
    MTD_PM_Cash_Chart=MTD_PM_Cash_Chart.astype(float)
    MTD_PM_Cash_Chart.plot(kind='bar',figsize=(20,10))
    plt.legend(title='MTD_PM_Cash')
    plt.title('MTD_PM_Cash')
    #save the plot as a picture
    plt.savefig(r'P:\All\FinanceTradingMO\Rebate_Borrow breakdown\plots/MTD_PM_Cash')
    plt.show()
    plt.close()
    return MTD_PM_Cash_Chart,plt
MTD_PM_Cash_Chart,plt=generate_MTDPM_cash_chart()