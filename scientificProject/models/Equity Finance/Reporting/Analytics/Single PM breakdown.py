...
import numpy as np
import pandas as pd
poly_fin = pd.read_csv('P:\All\FinanceTradingMO\Rebate_Borrow breakdwon\combineddata.csv',
...
#Filter Custodian Account that has the string of PMlist
poly_fin = poly_fin.loc[poly_fin['Custodian Account'].str.contains(pmlist)]
...
if check == 'yes':
    ...
    MTD_PB_Swap = raw_swap.groupby(['Custodian Code','BB Ticker','Month']).sum()
    # MTD_PM_Swap = raw_swap.groupby(['PM', raw_swap['Month']]).sum()
    MTD_PB_Swap = MTD_PB_Swap.reset_index()
    # MTD_PM_Swap = MTD_PM_Swap.reset_index()
    MTD_PB_Swap.columns = ['PB', 'Month','BB Ticker', 'MTD Short Fee Accrual', 'MTD Short Proceeds Rebate']
    # MTD_PM_Swap.columns = ['PM', 'BB Ticker','Month', 'MTD Short Fee Accrual', 'MTD Short Proceeds Rebate']
    ...
    with pd.ExcelWriter(r"P:\All\FinanceTradingMO\Rebate_Borrow breakdwon\202304_Financing_AKUNG_ytd.xlsx") as writer:
