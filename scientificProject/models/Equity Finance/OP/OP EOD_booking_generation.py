...
#replace Book name BORROW_FINANCE_ARB to PCM_B
OP_pos['Book Name'] = OP_pos['Book Name'].str.replace('BORROW_FINANCE_ARB', 'PCM_B')
...
def unwind_OP_trades_generation(OP_pos):
    ...
    #replcae PCM_BORROW to PCM_B under Internal Account
    Unwind_trades['Internal Account'] = Unwind_trades['Internal Account'].str.replace('PCM_BORROW', 'PCM_B')
        ...
        for j in range(len(OP_pos)):
            ...
            if Unwind_trades_X['ID'][i] == OP_pos['ID2'][j]:
                ...
                if Unwind_trades_X['unwind_op_quantity'][i] >= OP_pos['Position'][j]:
                    ...
                    Unwind_trades_X['Instrument Id'][i] = OP_pos['Instrument Id'][j]
                    Unwind_trades_X['Custodian Account Display Name'][i] = OP_pos['Custodian Account Display Name'][j]
                    Unwind_trades_X['unwind_op_quantity'][i] = 0
    ...
    #drop Enfusion Instrument Id column
    ...
    if not Unwind_trades_shell.empty:
...
def op_booking_generate():
    ...
    #check if there's empty value under Custodian Acct Id and Book ID,if yes, print a message "Please check the mapping file"
    if OP_new_trades['Custodian Acct Id'].isnull().values.any() == True:
        print('Please check the acct mapping file')
    if OP_new_trades['Book ID'].isnull().values.any() == True:
        print('Please check the book mapping file')
    ...
    if opbooking_trades_shell.empty == False:
        ...
        #append the
        # rename deduction to Notional Quantity
        #remove notional quantity column and replace it with real_op
        opbooking_trades_shell = opbooking_trades_shell.drop(['Notional Quantity'],axis=1)
        ...
        opbooking_trades_shell['Enfusion Instrument Id']=''
        ...
        opbooking_trades_shell = opbooking_trades_shell.drop(['First Payment Date'], axis=1)
        ...
        return opbooking_trades_shell,OP_new_trades
#run op_booking_generate
opbooking_trades_shell,OP_new_trades = op_booking_generate()
#save the opbooking_trades_shell to an excel file
#rearrange the indexes of the opbooking_trades_shell to match Unwind_trades_shell
#save the columns of Unwind_trades_shell to a list called op_file_columns
op_file_columns1 = list(Unwind_trades_shell.columns)
op_file_columns2=list(opbooking_trades_shell.columns)

#combine the two lists, keep the unique values of columns1
op_file_columns11 = list(set(op_file_columns1))
op_file_columns22 = list(set(op_file_columns2))
#re-arrange the columns of opbooking_trades_shell to match Unwind_trades_shell
#check the duplicated columns in op_file_columns



#
# #append the 2 dataframes
opbooking_trades_shell_combined = opbooking_trades_shell.append(Unwind_trades_shell)
#add another check if there's empty value under Book Id and Account Id,if yes, print a message "Please check the mapping file"
if opbooking_trades_shell_combined['Account Id'].isnull().values.any() == True:
    print('Please check the acct mapping file')
if opbooking_trades_shell_combined['Book Id'].isnull().values.any() == True:
    print('Please check the book mapping file')
#if not empty value then print "mapping all good"
else:
    print('mapping all good')

#save the opbooking_trades_shell_combined to an excel file
opbooking_trades_shell_combined.to_excel(r"P:\Operations\Polymer - Middle Office\PTH and FX booking\OP_EOD_INPUT/OP_Upload_" + today + '.xlsx', index=False)

#change the date format
from openpyxl import load_workbook

#do the same to the opbooking_trades_shell_combined
wb3= load_workbook(r"P:\Operations\Polymer - Middle Office\PTH and FX booking\OP_EOD_INPUT/OP_Upload_" + today + '.xlsx')
ws3 = wb3.active
for row in ws3.iter_rows(min_row=2, min_col=2, max_col=3):
    for cell in row:
...
wb3.save(r"P:\Operations\Polymer - Middle Office\PTH and FX booking\OP_EOD_INPUT/OP_Upload_" + today + '.xlsx')
